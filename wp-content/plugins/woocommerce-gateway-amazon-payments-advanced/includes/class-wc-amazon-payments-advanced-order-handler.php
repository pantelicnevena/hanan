    ...
    public function authorize_payment( $order_id, $amazon_reference_id, $capture_now = false ) {
		...
		if ( $order->payment_method == 'amazon_payments_advanced' ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				if ( 'declined' == $state ) {
					$order->add_order_note( sprintf( __( 'Order Declined with reason code: %s', 'woocommerce-gateway-amazon-payments-advanced' ), $response['AuthorizeResult']['AuthorizationDetails']['AuthorizationStatus']['ReasonCode'] ) );
